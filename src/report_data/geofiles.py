...
SCL_LANDSCAPES_SHP_FIELDS = (
    ...
    (
        "kba_connected_eff_pot_hab_area",
        "kceph_area",
    ),
    (
        "kba_eff_pot_hab_area",
        "keph_area",
    ),
    (
        "kba_indigenous_range_area",
        "kind_area",
    ),
    (
        "kba_occupied_eff_pot_hab_area",
        "koeph_area",
    ),
    (
        "kba_str_hab_area",
    ...
    (
        "pa_indigenous_range_area",
        "pind_area",
    ),
    (
        "pa_occupied_eff_pot_hab_area",
        "poeph_area",
    ),
    (
        "pa_str_hab_area",
...
def create_country_geojson(geojson_path, country_iso=None):
    country_iso = country_iso or "GLOBAL"
    geojson_path = Path(geojson_path)
    dir_path = geojson_path.parent
    file_name = f"{geojson_path.stem}_{country_iso}.geojson"
    output_path = Path(dir_path, file_name)
    ...
    if shell(
        "ogr2ogr",
        '-of "GeoJSON"',
        '-dialect "sqlite" ' f'-sql "select * from {geojson_path.stem}{where}"',
        str(output_path),
        str(geojson_path),
    ):
        return output_path
...
def generate_by_country(
    data_file_names, data_dir_path: Union[Path, str], country_iso: str
):
    geo_file_paths = []
    for data_file_name, fields in data_file_names:
        geojson_file_path = str(Path(data_dir_path, data_file_name))

        country_shapefile_path = create_country_shapefile(
            geojson_file_path, fields, country_iso
        )
        if country_shapefile_path is None:
            continue

        geo_file_paths.extend(get_shapefile_paths(country_shapefile_path))

        country_geojson_path = create_country_geojson(geojson_file_path, country_iso)
        if country_geojson_path is None:
            continue
...
def generate(
    ...
    with ProcessPoolExecutor() as exc:
        return exc.map(
            generate_by_country,
            itertools.repeat(data_file_names),
            itertools.repeat(data_dir_path),
            country_isos,
        )
